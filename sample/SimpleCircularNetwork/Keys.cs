namespace SimpleCircularNetwork
{
    /// <summary>
    /// This collaction of animals (Keys) has been secretly crafted to ensure no collisions when using our EightBitHashingService
    /// </summary>
    public static class Keys
    {
        public const string Abyssinian = "Abyssinian";
        public const string Alligator = "Alligator";
        public const string Anteater = "Anteater";
        public const string Armadillo = "Armadillo";
        public const string Axolotl = "Axolotl";
        public const string Barb = "Barb";
        public const string Barracuda = "Barracuda";
        public const string Bison = "Bison";
        public const string Buffalo = "Buffalo";
        public const string Bulldog = "Bulldog";
        public const string Butterfly = "Butterfly";
        public const string Cassowary = "Cassowary";
        public const string Cattle = "Cattle";
        public const string Centipede = "Centipede";
        public const string Chinook = "Chinook";
        public const string Clam = "Clam";
        public const string Coati = "Coati";
        public const string Cockroach = "Cockroach";
        public const string Coral = "Coral";
        public const string Coyote = "Coyote";
        public const string Crow = "Crow";
        public const string Cuttlefish = "Cuttlefish";
        public const string Dhole = "Dhole";
        public const string Dodo = "Dodo";
        public const string Dragonfly = "Dragonfly";
        public const string Eagle = "Eagle";
        public const string Earwig = "Earwig";
        public const string Eland = "Eland";
        public const string Ferret = "Ferret";
        public const string Gibbon = "Gibbon";
        public const string Giraffe = "Giraffe";
        public const string Goldfinch = "Goldfinch";
        public const string Grasshopper = "Grasshopper";
        public const string Greyhound = "Greyhound";
        public const string Gull = "Gull";
        public const string Hare = "Hare";
        public const string Hedgehog = "Hedgehog";
        public const string Herring = "Herring";
        public const string Himalayan = "Himalayan";
        public const string Ibis = "Ibis";
        public const string Jaguar = "Jaguar";
        public const string Kiwi = "Kiwi";
        public const string Lemming = "Lemming";
        public const string Lemur = "Lemur";
        public const string Lynx = "Lynx";
        public const string Mallard = "Mallard";
        public const string Mastiff = "Mastiff";
        public const string Meerkat = "Meerkat";
        public const string Mole = "Mole";
        public const string Molly = "Molly";
        public const string Mosquito = "Mosquito";
        public const string Mule = "Mule";
        public const string Nightingale = "Nightingale";
        public const string Okapi = "Okapi";
        public const string Olm = "Olm";
        public const string Peacock = "Peacock";
        public const string Pony = "Pony";
        public const string Quetzal = "Quetzal";
        public const string Quokka = "Quokka";
        public const string Rail = "Rail";
        public const string Rat = "Rat";
        public const string Rottweiler = "Rottweiler";
        public const string Ruff = "Ruff";
        public const string Sandpiper = "Sandpiper";
        public const string Seahorse = "Seahorse";
        public const string Sheep = "Sheep";
        public const string Snake = "Snake";
        public const string Sparrow = "Sparrow";
        public const string Sponge = "Sponge";
        public const string Squirrel = "Squirrel";
        public const string Tiffany = "Tiffany";
        public const string Tiger = "Tiger";
        public const string Toad = "Toad";
        public const string Tuatara = "Tuatara";
        public const string Turkey = "Turkey";
        public const string Turtle = "Turtle";
        public const string Viper = "Viper";
        public const string Walrus = "Walrus";
        public const string Weasel = "Weasel";
        public const string Whippet = "Whippet";
        public const string Wombat = "Wombat";
        public const string Woodpecker = "Woodpecker";
        public const string Wren = "Wren";
        public const string Yak = "Yak";


        public static string[] Animals =
        {
            Abyssinian,
            Alligator,
            Anteater,
            Armadillo,
            Axolotl,
            Barb,
            Barracuda,
            Bison,
            Buffalo,
            Bulldog,
            Butterfly,
            Cassowary,
            Cattle,
            Centipede,
            Chinook,
            Clam,
            Coati,
            Cockroach,
            Coral,
            Coyote,
            Crow,
            Cuttlefish,
            Dhole,
            Dodo,
            Dragonfly,
            Eagle,
            Earwig,
            Eland,
            Ferret,
            Gibbon,
            Giraffe,
            Goldfinch,
            Grasshopper,
            Greyhound,
            Gull,
            Hare,
            Hedgehog,
            Herring,
            Himalayan,
            Ibis,
            Jaguar,
            Kiwi,
            Lemming,
            Lemur,
            Lynx,
            Mallard,
            Mastiff,
            Meerkat,
            Mole,
            Molly,
            Mosquito,
            Mule,
            Nightingale,
            Okapi,
            Olm,
            Peacock,
            Pony,
            Quetzal,
            Quokka,
            Rail,
            Rat,
            Rottweiler,
            Ruff,
            Sandpiper,
            Seahorse,
            Sheep,
            Snake,
            Sparrow,
            Sponge,
            Squirrel,
            Tiffany,
            Tiger,
            Toad,
            Tuatara,
            Turkey,
            Turtle,
            Viper,
            Walrus,
            Weasel,
            Whippet,
            Wombat,
            Woodpecker,
            Wren,
            Yak,
        };
    }
}